...
**Java 17 · Spring Boot 4.0.7 · Koog 1.0.0 (Java API) · Spring AI 2.0.0 (OpenAI-compatible, configurable base URL) · Maven**
...
1. **Deterministic control flow, agentic stages.** The *sequence* of the
   pipeline is plain Java (`InvoicePipeline`, `@Async` on a dedicated,
   bounded executor); Koog agents reason only *within* a stage (which vendor
   to look up, how to summarise findings, how to react to a failed Geneva
   push). Prompts never decide whether an invoice may be paid.
2. **Guards in code, not prompts.** Segregation of duties (`ReviewService`),
   ...
   a Geneva reference" check (`PostingTools`) are enforced in Java. Tools
...
including every agent tool call, immutable domain records with a copy-on-write
builder, and RFC 7807 problem details.
...
|---|---|
| `agent` | Koog agent factory (Java builder API) + system prompts |
| `agent.tools` | Koog `ToolSet`s (`@Tool`/`@LLMDescription`) — validation lookups, Geneva push, control pack, client notification; all state-guarded |
| `service` | `InvoicePipeline` (async agent stages), submission service, four-eye review, NAV control pack |
...
| `api` | REST controller, record DTOs, RFC 7807 error handling |
...
- Koog notes: the `@Tool`/`@LLMDescription` tool API and the `AIAgent`
  builder come from the stable Koog 1.0 Java surface; `agent.run(...)` is
  plain blocking from Java (Koog manages dispatching internally). If your
  build resolves different symbol locations, check the Koog 1.0 migration
  notes — 1.0 removed all previously deprecated APIs.
